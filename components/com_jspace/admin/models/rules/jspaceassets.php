...
class JFormRuleJSpaceAssets extends JFormRule
{
    public function test($element, $value, $group = null, $input = null, $form = null)
    {
		...
		$derivatives = JArrayHelper::getValue($collection, (string)$element->attributes()->name, array(), 'array');

        foreach ($derivatives as $asset)
        {
		...
		if (array_key_exists((string)$element->attributes()->name, $collection))
		{
			...
			do
			{
                $derivatives = JArrayHelper::getValue($collection, current($keys), array(), 'array');
