<?php
class JSpaceTableDataObject extends JTable
{
	/**
	 * Constructor
	 *
	...
	public function store($updateNulls = false)
	{
		...
		if ($result) 
		{
			if ($this->id > 0)
			{
				...
				$table = JTable::getInstance('DataObjectAncestor', 'JSpaceTable');
	...
	public function check()
	{
		...
		if (trim($this->title) == '')
		{
			$this->setError(JText::_('COM_JSPACE_WARNING_PROVIDE_VALID_TITLE'));
		...
		if (trim($this->title) == '')
		{
			$this->setError(JText::_('COM_JSPACE_WARNING_PROVIDE_VALID_NAME'));
	...
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		foreach ($pks as $pk)
		{
			...
			if ($checkoutSupport)
			{
				...
				$query->clear()
				->select('COUNT(' . $k . ')')
				->from($this->_tbl.' AS do')
				->join('INNER', ' #__jspace_dataobject_ancestors AS doa ON (do.id = doa.ancestor)')
				->where('doa.ancestor = '.$node->id.' AND NOT doa.decendant = '.$node->id)
			...
			if ($node->parent_id)
			{
				...
				->from($this->_tbl.' AS do')
				->join('INNER', ' #__jspace_dataobject_ancestors AS doa ON (do.id = doa.ancestor)')
				->where('doa.decendant = '.$node->id.' AND NOT doa.ancestor = '.$node->id)
				->where('do.published < ' . (int) $compareState);
			...
			$query->clear()
			->update($this->_db->qn($this->_tbl, 'do'))
			->join('INNER',
				$this->_db->qn('#__jspace_dataobject_ancestors', 'doa').
				' ON (do.'.$this->_db->qn('id').'=doa.'.$this->_db->qn('decendant').')')
			->set('do.'.$this->_db->qn('published').' = '.(int) $state)
			->where('doa.'.$this->_db->qn('ancestor').'='.(int)$pk);
	...
	public function delete($pk = null, $children = true)
	{
		...
		if ($children)
		{
			...
			$query = 
			"DELETE do FROM ".$this->_db->qn($this->_tbl, 'do')." ".
			"INNER JOIN ". 
				$this->_db->qn('#__jspace_dataobject_ancestors', 'doa')." ".
				"ON (do.".$this->_db->qn('id')."=doa.".$this->_db->qn('decendant').") ".
			"WHERE doa.ancestor = ".$node->id.";";
			...
			$ancestor = JTable::getInstance('DataObjectAncestor', 'JSpaceTable');
			$ancestor->delete(array('ancestor'=>$node->id));
			
			$dataobjectCategory = JTable::getInstance('DataObjectCategory', 'JSpaceTable');
			$dataobjectCategory->delete(array("dataobject_id"=>$node->id));
		}
	...
	protected function _getAssetName()
	{
		...
		return 'com_jspace.dataobject.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$dataobjectCategory = JTable::getInstance('DataObjectCategory', 'JSpaceTable');
		$dataobjectCategory->load(array("dataobject_id"=>$this->id));

		// This is a article under a category.
		if ($dataobjectCategory->catid)
		{
			// Build the query to get the asset id for the parent category.
			$query = $this->_db->getQuery(true)
			->select(`c.`.$this->_db->qn('asset_id'))
			->from($this->_db->qn('#__jspace_dataobjects_categories', 'doc'))
			->join('INNER', $this->_db->qn('#__categories', 'c').' ON (c.id = doc.catid)')
			->where('doc.'.$this->_db->qn('dataobject_id') . ' = ' . (int) $this->id);
		...
		{
			// Build the query to get the asset id for the parent data object.
			$query = $this->_db->getQuery(true)
			->select(`do.`.$this->_db->qn('asset_id'))
			->from($this->_db->qn('#__jspace_dataobjects', 'do'))
			->where('do.'.$this->_db->qn('id') . ' = ' . (int) $this->parent_id);
