 ...
 */
namespace JSpace\Oai;

use \JSpace\Factory;
use \JSpace\Archive\Record;

/**
 * Provides an Oai request and its associated output.
 */
class Request extends \JObject
{
            ...
            if (array_search($verb, array_keys(self::$verbs), true) !== false) {
                ...
                if ($this->get('resumptionToken')) {
                    ...
                    if (!$resumptionToken instanceof stdClass) {
                        throw new \InvalidArgumentException('badResumptionToken');
                    }
                ...
                throw new \InvalidArgumentException('badVerb');
            }
        } catch (Exception $e) {
            if ($e instanceof \InvalidArgumentException) {
                $error = $response->createElement("error", JText::_('LIB_JSPACE_OAI_'.\JString::strtoupper($e->getMessage()).'_LABEL'));
                $error->setAttribute('code', $e->getMessage());
     ...
     *
     * @exception  InvalidArgumentException  Thrown if no metadata formats exist or
    ...
    protected function listMetadataFormats()
    {
        ...
        if (count($formats) === 0) {
            throw new \InvalidArgumentException('noMetadataFormats');
        }

        // if there is an identifier, validate it.
        if (isset($this->identifier)) {
            if (!($identifier = $this->lookupIdentifier())) {
                throw new \InvalidArgumentException('idDoesNotExist');
            }
        }
    ...
    protected function listSets()
    {
        if ($this->getCategoryCount() == 0) {
            ...
            throw new \InvalidArgumentException('noRecordsMatch');
        }
    ...
    private function getCategoryQuery()
    {
        ...
        $query
            ...
            ->where($db->qn('c.access').'='.\JSpace\Factory::getConfig()->get('oai_accesslevel', 1))
            ->where($db->qn('c.published').' IN (1, 2)')
            ->order($db->qn('c.lft').' ASC');

        // Also ignore parent categories.
        $ignoreCategories = \JSpace\Factory::getConfig()->get('oai_ignorecategories', array());
    ...
    private function getRecordQuery()
    {
        ...
        $where[] = $db->qn('r.access').'='.\JSpace\Factory::getConfig()->get('oai_accesslevel', 1);
        $where[] = $db->qn('c.access').'='.\JSpace\Factory::getConfig()->get('oai_accesslevel', 1);

        $nullDate = $db->q($db->getNullDate());

        $datestamp = \JSpace\Factory::getConfig()->get('oai_earliest_datestamp', '1970-01-01');
     ...
     * identifier has an invalid format.
     *
     * @return  JSpace\Archive\Record  The record if the identifier is found, null otherwise.
     */
    private function lookupRecord()
    {
        ...
        $record = Record::getInstance(\JArrayHelper::getValue($parts, count($parts)-1));
     ...
     *
     * @throw  InvalidArgumentException  Thrown if no JOAI plugin is configured to
