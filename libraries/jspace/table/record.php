<?php
jimport('jspace.table.recordancestor');
jimport('jspace.table.recordcategory');
...
$state, $userId)
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$recordCategory = JTable::getInstance('RecordCategory', 'JSpaceTable');
		$recordCategory->load(array("record_id"=>$this->id));

		// This is a article under a category.
		if ($recordCategory->catid)
		{
			// Build the query to get the asset id for the parent category.
			$query = $this->_db->getQuery(true)
			->select(`c.`.$this->_db->qn('asset_id'))
			->from($this->_db->qn('#__jspace_record_categories', 'rc'))
			->join('INNER', $this->_db->qn('#__categories', 'c').' ON (c.id = rc.catid)')
			->where('rc.'.$this->_db->qn('record_id') . ' = ' . (int) $this->id);
		...
		{
			// Build the query to get the asset id for the parent record.
			$query = $this->_db->getQuery(true)
			->select(`r.`.$this->_db->qn('asset_id'))
			->from($this->_db->qn('#__jspace_records', 'r'))
			->where('r.'.$this->_db->qn('id') . ' = ' . (int) $this->parent_id);
