...
class SwordAppEntry {
    ...
    function buildhierarchy($sac_dr, $sac_ns) {
        ...
        $this->sac_title = SwordUtils::sac_clean($sac_dr->children($sac_ns['atom'])->title);
        $this->sac_summary = SwordUtils::sac_clean($sac_dr->children($sac_ns['atom'])->summary);

        // Store the updated date
        $this->sac_updated = $sac_dr->children($sac_ns['atom'])->updated;

        // Store the rights
        $this->sac_rights = SwordUtils::sac_clean($sac_dr->children($sac_ns['atom'])->rights);

        // Store the treatment
        $this->sac_treatment = SwordUtils::sac_clean($sac_dr->children($sac_ns['sword'])->treatment);

        // Store the verboseDescription
        $this->sac_verbose_treatment = SwordUtils::sac_clean($sac_dr->children($sac_ns['sword'])->verboseDescription);

        // Store the format namespace
        $this->sac_packaging = $sac_dr->children($sac_ns['sword'])->packaging;

        // Store the generator
        $this->sac_generator = SwordUtils::sac_clean($sac_dr->children($sac_ns['atom'])->generator);
        $sac_gen = $sac_dr->xpath("atom:generator");
        if (!empty($sac_gen)) { $this->sac_generator_uri = $sac_gen[0]->attributes()->uri; }

        // Store the user agent
        $this->sac_useragent = SwordUtils::sac_clean($sac_dr->children($sac_ns['sword'])->userAgent);
