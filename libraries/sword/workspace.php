...
jimport('sword.collection');
jimport("sword.utils");

class SwordWorkspace {
	...
	function buildhierarchy($sac_colls, $sac_ns) {
		...
		foreach ($sac_colls as $sac_collection) {
			...
			$sac_newcollection = new SwordCollection(SwordUtils::sac_clean($sac_collection->children($sac_ns['atom'])->title));
			...
			$sac_newcollection->sac_collpolicy = SwordUtils::sac_clean($sac_collection->children($sac_ns['sword'])->collectionPolicy);
			...
			if (array_key_exists('dcterms', $sac_ns)) {
				$sac_newcollection->sac_abstract = SwordUtils::sac_clean($sac_collection->children($sac_ns['dcterms'])->abstract);
			}
			...
			$sac_newcollection->sac_service = SwordUtils::sac_clean($sac_collection->children($sac_ns['sword'])->service);
